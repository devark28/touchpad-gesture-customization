    ...
    class TouchpadPinchGesture extends GObject.Object {
        ...
        constructor(params: {
            ...
            this.PINCH_MULTIPLIER =
                TouchpadConstants.PINCH_MULTIPLIER * (params.pinchSpeed ?? 1.0);
        }
        ...
        _handleEvent(
            ...
            if (
                ...
                (this._allowedModes & Main.actionMode) === 0
            ) {
                this._interrupt();
        ...
        public confirmPinch(
            ...
            if (this._ackState !== GestureACKState.PENDING_ACK) return;

            this._snapPoints = snapPoints;
            this._initialProgress = currentProgress;
            this._progress_scale = Math.clamp(
                currentProgress,
                ...this._getBounds()
            );
        ...
        private _interrupt() {
            if (this._ackState !== GestureACKState.ACKED) return;
        ...
        private _emitEnd(time: number) {
            ...
            let duration = Math.abs(
                ((this._progress_scale - endProgress) / velocity) *
                    DURATION_MULTIPLIER
            );
            duration = Math.clamp(
                duration,
        ...
        private _findPreviousPoint(pos: number) {
            const reversedIndex = this._snapPoints
                .slice()
                .reverse()
                .findIndex(p => p <= pos);
        ...
        private _getEndProgress(velocity: number) {
            ...
            const slope =
                DECELERATION_TOUCHPAD / (1.0 - DECELERATION_TOUCHPAD) / 1000.0;
