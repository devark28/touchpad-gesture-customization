...
import {SwipeTracker, CustomEventType} from 'resource:///org/gnome/shell/ui/swipeTracker.js';
...
export const TouchpadSwipeGesture = GObject.registerClass({
    ...
    _handleEvent(_actor: undefined | Clutter.Actor, event: CustomEventType): boolean {
        ...
        if ((this._allowedModes !== Shell.ActionMode.ALL) && ((this._allowedModes & actionMode) === 0)) {
    ...
    isItHoldAndSwipeGesture() {
        ...
        return (
            (this._holdGestureCancelTime - this._holdGestureBeginTime >= TouchpadConstants.HOLD_SWIPE_DELAY_DURATION) &&
			(this._swipeGestureBeginTime - this._holdGestureCancelTime <= Math.max(100, TouchpadConstants.HOLD_SWIPE_DELAY_DURATION))
        );
    }
