...
import {ForwardBackGestureExtension, type AppForwardBackKeyBinds} from './src/forwardBack.js';
...
export default class TouchpadGestureCustomization extends Extension {
    ...
    _initializeSettings() {
        if (this.settings) {
            ...
            Constants.ExtSettings.DEFAULT_OVERVIEW_GESTURE_DIRECTION = this.settings.get_boolean('default-overview-gesture-direction');
            ...
            Constants.TouchpadConstants.PINCH_MULTIPLIER = Constants.TouchpadConstants.DEFAULT_PINCH_MULTIPLIER * this.settings.get_double('touchpad-pinch-speed');
            Constants.AltTabConstants.DELAY_DURATION = this.settings.get_int('alttab-delay');
            Constants.TouchpadConstants.HOLD_SWIPE_DELAY_DURATION = this.settings.get_int('hold-swipe-delay-duration');
        }
    }
